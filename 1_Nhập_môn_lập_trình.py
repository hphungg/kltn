...
USER_DATA_FILE = st.secrets["USER_DATA_FILE"] + ".json"
...
if "messages" not in st.session_state:
    st.session_state.messages = load_chat_history("1")
...
def handle_knowledge_suggestion(suggestion):
    ...
    save_chat_history(st.session_state.messages, "1")
...
if (("messages" not in st.session_state) or (len(st.session_state.messages) == 0)):
    ...
    for i, question in enumerate(question_suggestions):
        st.button(question["title"], key=f"start_qs_{i}", on_click=lambda q=question: handle_question_suggestion(q))
...
for i, question in enumerate(question_suggestions):
    st.sidebar.button(question["title"], key=f"side_qs_{i}", on_click=lambda q=question: handle_question_suggestion(q))
